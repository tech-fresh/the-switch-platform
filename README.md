...
- Added **do not replace or shorten** preservation rule on all copies. Earlier build history and completion records were not removed — list text was added back alongside existing sections.
- `PLATFORM-GUIDE.md` remains the merged operator guide; the four files now mirror the same final list.
...
- Replaced duplicate 22-item list in this README with a pointer to `PLATFORM-GUIDE.md`. *(Later the same day: full list restored in README with preservation rule — see entry above.)*
...
**Final list — do not replace or shorten.** This is the only list for judging true 100% completion. Do not overwrite earlier build history or completion records in this README; only append status updates and evidence notes. The same list is mirrored in `AGENTS.md`, `HANDOFF.md`, and `PLATFORM-GUIDE.md` — keep all copies aligned when an item's status changes.
