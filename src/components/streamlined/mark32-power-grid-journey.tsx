...
export function Mark32PowerGridJourney({
  ...
  return (
    ...
    >
              ...
              <li key={level} className="flex min-w-[4.5rem] flex-1 flex-col items-center gap-2 text-center">
                <span
                  className={`grid size-10 shrink-0 place-items-center rounded-full text-xs font-bold transition ${
                    isCurrent
                      ? "size-12 bg-teal-800 text-white shadow-lg ring-4 ring-teal-200"
                      : isComplete
      ...
      <div className="mt-6 border border-teal-800 bg-gradient-to-br from-teal-900 to-teal-800 p-4 text-white sm:p-5">
        <div className="flex flex-wrap items-center gap-4">
          <span className="inline-flex size-12 items-center justify-center rounded-2xl bg-teal-700 text-xl shadow-lg">
            ◆
          </span>
          <div>
            <p className="text-sm font-semibold text-teal-100">Current level</p>
        ...
        <p className="mt-3 text-sm text-teal-100">Keep answering to level up!</p>
      </div>
    </section>
  );
}
