...
export function buildOperationsOverview(input: BuildOperationsOverviewInput): OperationsOverview {
    ...
    },
    {
      ...
      headline:
        input.persistenceDriver === "memory"
          ...
          : input.isPrototypePersistence
            ? "Local file persistence is still carrying launch data."
      ...
      detail:
        input.persistenceDriver === "memory"
          ...
          : input.isPrototypePersistence
            ? `Data is currently written to ${input.dataDirectory}, so backup and restore checks should stay visible.`
  ...
  if (input.persistenceDriver === "memory") {
    ...
    alerts.push({
      ...
      detail: `Launch data is currently tied to ${input.dataDirectory}, so recovery procedures should stay visible.`,
